...
df_electricload = pd.DataFrame({"time": date_range, "Node1": df_profiles["LOAD"]})
...
def summer_weight(a):
    return [1.0 + a * np.sin(-np.pi * i / n_summer_steps) for i in range(n_summer_steps)]


df_nuclear = pd.DataFrame(index=date_range)
df_nuclear.index.name = "time"
df_nuclear["Node1"] = 1.0
for t, v in zip(summer_range, summer_weight(0.35)):
    df_nuclear.loc[t, "Node1"] = v

print("Average availability Nuclear:\n", df_nuclear.mean())

df_nuclear.reset_index().to_csv(scenario_data_path / "nuclear.csv", index=False, date_format=datetime_format)
...
if __name__ == "__main__":
    ...
    fig = go.Figure(
        data=[
            go.Scatter(y=df_profiles["ONWP"], name="ONWP"),
            go.Scatter(y=df_profiles["LOAD"], name="LOAD"),
            go.Scatter(y=df_profiles["OFWP"], name="OFWP"),
        ]
    )
